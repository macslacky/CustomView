...
/*
    public ArrayList<ClassBankAccount> getAllBank() {
        ArrayList<ClassBankAccount> arrayListBank = new ArrayList<>();
        ClassBankAccount cursorBankAccount;

        SQLiteDatabase sqlSaldoConti = this.getReadableDatabase();

        String stringSql = "select * FROM " + CONTACTS_TABLE_NAME;
        Cursor cursor = sqlSaldoConti.rawQuery(stringSql, null);

        //cursorBankAccount = new ClassBankAccount("nome banca", "saldo iniziale");
        cursor.moveToFirst();
        while (cursor.isAfterLast() == false) {
            //cursorBankAccount.nomeBanca = cursor.getString(cursor.getColumnIndex(CONTACTS_COLUMN_NOME));
            //cursorBankAccount.saldoIniziale = cursor.getString(cursor.getColumnIndex(CONTACTS_COLUMN_SALDO_INIZIALE));
            //arrayListBank.add(cursorBankAccount);
            cursor.moveToFirst();
        }
